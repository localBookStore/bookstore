	...
	return (
      ...
      <CouponTitle>🎫 쿠폰 등록하기</CouponTitle> 
			<form onSubmit={handleSubmit(submitEvent)}>
				...
				<Input variant="outlined" placeholder="쿠폰 이름" {...register('name', {required: true, maxLength: 80 })} />
        <div>{errors.name && "쿠폰이름은 필수이며 80자 이내로 입력하세요"}</div>
				<ContentDiv>쿠폰에 대한 설명을 입력하세요. </ContentDiv>
				<Input variant="outlined" placeholder="쿠폰 설명" {...register('description', {required:true})} />
        <ContentDiv>할인율(%)을 입력하세요. (숫자만 입력)</ContentDiv>
        <Input variant="outlined" placeholder="할인율" {...register('discountRate', {required:true, pattern:/^[0-9]*$/})}/>
        <div>{errors.discountRate && "할인율은 필수이며 숫자만 입력하세요"}</div>
        <ContentDiv>종료일자를 입력하세요(년월일기준 "-" 기재). (ex. 2020-02-02) </ContentDiv>
        <Input variant="outlined" placeholder="쿠폰 종료일자" {...register('endDate')}/>
